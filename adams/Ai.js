const {
  adams
} = require("../Ibrahim/adams"); const axios = require("axios"); const config = require("../config");
...
async function puterChat(prompt, model = "gpt-4.1-nano") {
  try {
    const response = await axios.post("https://api.puter.com/v2/chat", {
      model, prompt
    }, {
      headers: {
        "Content-Type": "application/json"
      }
    }); return response.data.output || "⚠️ No response from AI.";
  } catch (error) {
    console.error("Puter API error:", error.message); throw new Error("AI request failed.");
  }
}
...
aiCommands.forEach(cmd => {
  adams( {
    nomCom: cmd.nomCom, aliases: cmd.aliases, categorie: cmd.categorie, reaction: cmd.reaction, description: cmd.description
  }, async (dest, zk, commandOptions) => {
    const {
      arg, ms, repondre
    } = commandOptions; const sender = ms.key.remoteJid; const input = arg.join(" ").trim();
...
// Clear AI memory
adams( {
  nomCom: "resetai", aliases: ["aiclear"], categorie: "AI", reaction: "🧼", description: "Clear AI memory for this chat"
}, async (dest, zk, commandOptions) => {
  const jid = commandOptions?.ms?.key?.remoteJid; sessionStore.delete(jid); await commandOptions.repondre("✅ Memory cleared. VeilWolf-AI is reset for this session.");
});

// Help Command
adams( {
  nomCom: "aihelp", aliases: ["helpai", "aicmds"], categorie: "AI", reaction: "❓"
}, async (dest, zk, commandOptions) => {
  const {
    repondre
  } = commandOptions;
